...
The ordinary product surface exposes `RADAR`, `ONE_TAP` and `AUTO`; `SHADOW_AUTO` is isolated under developer settings. Refresh is an independent `OFF` / `ALERT_ONLY` / `AUTO_REFRESH` setting and defaults to `OFF`. The Shadow coordinator owns deterministic single-target selection, monotonic delay, fresh-frame revalidation and policy completion. It has no input API. Radar alerts may accept high-confidence evidence when participant OCR is unknown, while the action policy always remains strict. A low-level Accessibility bridge accepts only unexpired, purpose-scoped refresh requests for the locally verified foreground package and a confirmed event-list screen. It cannot retrieve window content and contains no selection, join or send logic. `ONE_TAP` and `AUTO` join/send actions remain release-gated.
